...
class Figures {
	constructor(name, constX, constY) {
		...
		this.image.src = "/images/games/roulette/figures/" + name + ".png";
		switch (this.name) {
			case "circle":
				this.price = 1;
				break;
...
function getText() {
	...
	for (let i = 0; i < thisFigures.length; i++) {
		img.src = "/images/games/roulette/figures/" + thisFigures[i] + ".png"
		if (i > 4) {
...
setTimeout(function() {
	...
	ground.src = "/images/games/roulette/ground2.png";	// Загружаем текстуру поля
...
async function game() {
	...
	await new Promise(r => setTimeout(r, 1200));					// Пауза на 1200мс (await ждет возращение промиса для продолжения)
	...
	do {
		...
		if (spin) {
			...
			for (let i = 0; i < 6; i++) {
				...
				for (let newY = 0; newY < randFig[i].length; newY++) {
	...
	// POST запрос на сервер
	let url = '/game/roulette/game-over';
	thisSum -= 100;

	axios.defaults.headers.common['X-CSRF-TOKEN'] = csrf; // Устанавливаем заголовок для всех запросов

	axios.post(url, { score: thisSum, token: token })
		.then(function(response) {
			deposit = Number(response.data);
		})
		.catch(function (error) {
